...
// mfeeney (at) fanshawec (d0t) ca
// It may be distributed under the terms of the MIT License:
// https://opensource.org/licenses/MIT
// Use this code at your own risk. It is intended only as a learning aid.
...
public:
	...
	struct sSaveFileParams
	{
		bool bIncludeNormals = true;
		bool bInculdeRGBA = true;
		bool bIncludeUVs = true;
		bool bIncludeTangents = false;
		bool bIncludeBinormal = false;
		std::vector<std::wstring> vecComments;
	};
	bool SavePlyFileASCII(std::wstring fileName, bool bOverwrite, sSaveFileParams saveFileParams);
	// This one does the actual saving, and allows you to pass any created or updated info to be saved...
	bool SavePlyFileASCII(std::wstring fileName, std::vector<PlyVertex> vecVerts, std::vector<PlyElement> vecTris, bool bOverwrite, sSaveFileParams saveFileParams);
	...
	void FixOutOfBoundsHogwartsUVs(void);
	void SetUniformColour(CVector3f RGB);
