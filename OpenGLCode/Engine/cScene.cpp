...
void cScene::RemoveEnemy(iEnemy* enemy)
{
    for (int i = 0; i < enemies.size(); ++i)
    {
        if (enemies[i]->GetEnemyMesh()->GetUniqueID() == enemy->GetEnemyMesh()->GetUniqueID())
        {
    ...
    DeleteMeshById(enemy->GetEnemyMesh()->GetUniqueID());
}

void cScene::RemoveBullet(iBullet* bullet)
{
    for (int i = 0; i < sceneBullets.size(); ++i)
    {
        if (sceneBullets[i]->GetBulletMesh()->GetUniqueID() == bullet->GetBulletMesh()->GetUniqueID())
        {
            sceneBullets.erase(sceneBullets.begin() + i);
            --i;
        }
    }

    cMesh* bulletMesh = bullet->GetBulletMesh();
    DeleteMeshById(bulletMesh->GetUniqueID());
}

void cScene::ClearStage()
{
    for (int i = 0; i < sceneBullets.size(); ++i)
    {
        cMesh* bulletMesh = sceneBullets[i]->GetBulletMesh();
        DeleteMeshById(bulletMesh->GetUniqueID());
    }
    ...
    for (int i = 0; i < enemies.size(); i++)
    {
        cMesh* enemyMesh = enemies[i]->GetEnemyMesh();
        DeleteMeshById(enemyMesh->GetUniqueID());
    }
...
void cScene::PauseGame()
{
}

void cScene::ResumeGame()
{
}
...
void cScene::StartScene()
{
    ...
    g_camera = GetCameras()[1];

    g_animationSystem->StartAllAnimations(GetGameObjects());
