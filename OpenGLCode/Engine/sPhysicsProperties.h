...
struct sPhysicsProperties
{
	...
	sPhysicsProperties(); 
	sPhysicsProperties(eShape shapeType);
...
#pragma region Shape_Type_Structures
	...
	struct sCone
	{
		sCone(float height, float bottomRadius)
		{
			m_height = height;
...
//	glm::vec3 orientation = glm::vec3(0.0f);
	...
	{
		m_qOrientation = glm::normalize(rotation);
	}
...
private:
	...
	float m_gravity = (9.81f / 5.0f);////    pGroundMeshShape->acceleration.y = (-9.81f / 5.0f);
public:
	...
	bool RemoveForce(std::string friendlyName);
	void ClearForces();
