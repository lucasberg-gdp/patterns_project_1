...
//	static int l_GetObjectState( lua_State *L );
    ...
    static void ChangeTaylorSwiftTexture(std::string newTexture);

    // Here the same sort of thing, but that Lua can call
    static int lua_ChangeTaylorSwiftTexture(lua_State* L);

    // Here the same sort of thing, but that Lua can call
	static int lua_AddSerialMoveObjectCommand(lua_State* L);

	static int lua_GetMeshPositionByFriendlyName(lua_State* L);

	static int lua_SetMeshPositionByFriendlyName(lua_State* L);
