...
bool CPlyFile5nt::SavePlyFileASCII(std::wstring fileName, bool bOverwrite, sSaveFileParams saveFileParams)
{
	return this->SavePlyFileASCII(fileName, this->m_verticies, this->m_elements, bOverwrite, saveFileParams);
}


bool CPlyFile5nt::SavePlyFileASCII(std::wstring fileName, std::vector<PlyVertex> vecVerts, std::vector<PlyElement> vecTris,
								   bool bOverwrite, sSaveFileParams saveFileParams)
{
	// Try to open this file for reading (to see if it's there)
	std::wifstream testFile(fileName);
	if ( testFile.good() )
	{
		// File exists
		if ( !bOverwrite )
		{
			// But we can't overwrite
			return false;
		}
		// Exists, but we can overwrite
	}

	testFile.close();

	std::wofstream theFile(fileName);

	// Print header:
	theFile << "ply" << std::endl;
	theFile << "format ascii 1.0" << std::endl;
//		comment INFO-3111 Flatenator generated
//		comment TextureFile Dungeons_Texture_01.png
	for ( std::wstring &theComment : saveFileParams.vecComments )
	{
		theFile << L"comment " << theComment << std::endl;
	}

	theFile << "element vertex " << vecVerts.size() << std::endl;
	theFile << "property float x" << std::endl;
	theFile << "property float y" << std::endl;
	theFile << "property float z" << std::endl;

	if (saveFileParams.bIncludeNormals)
	{
		theFile << "property float nx" << std::endl;
		theFile << "property float ny" << std::endl;
		theFile << "property float nz" << std::endl;
	}

	if (saveFileParams.bInculdeRGBA)
	{
		theFile << "property uchar red" << std::endl;
		theFile << "property uchar green" << std::endl;
		theFile << "property uchar blue" << std::endl;
		theFile << "property uchar alpha" << std::endl;
	}

	if (saveFileParams.bIncludeUVs)
	{
		theFile << "property float texture_u" << std::endl;
		theFile << "property float texture_v" << std::endl;
	}

	theFile << "element face " << vecTris.size() << std::endl;

	theFile << "property list uchar int vertex_indices" << std::endl;
	theFile << "end_header" << std::endl;

	// Now output the vertex information
	for (PlyVertex& tempVert : vecVerts )
	{
		theFile << tempVert.xyz.x << " " << tempVert.xyz.y << " " << tempVert.xyz.z << " ";
		...
		{
			theFile
				<< (int)(tempVert.red * 255) << " "
				<< (int)(tempVert.green * 255) << " "
				<< (int)(tempVert.blue * 255) << " "
				<< (int)(tempVert.alpha * 255) << " ";
		}

		if (saveFileParams.bIncludeUVs)
		{
			theFile << tempVert.tex0u << " " << tempVert.tex0v << " ";
		}

		if (saveFileParams.bIncludeTangents)
		{
			theFile << tempVert.tangent.x << " " << tempVert.tangent.y << " " << tempVert.tangent.z << " ";
		}

		if (saveFileParams.bIncludeBinormal)
		{
			theFile << tempVert.binormal.x << " " << tempVert.binormal.y << " " << tempVert.binormal.z << " ";
		}

		theFile << std::endl;
	}//for (...vecVerts )

	// Now the triangles (elements)
	for ( PlyElement& curTri : vecTris )
	{
		theFile << "3 "
			<< curTri.vertex_index_1 << " "
			<< curTri.vertex_index_2 << " "
			<< curTri.vertex_index_3 << std::endl;
	}//for (...vecTris )
